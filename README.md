...
- 프리셋 설정을 `presets.json`으로 외부 파일 분리 (손쉬운 커스텀)
- JPG / PNG / WebP / HEIC 포맷 변환 (HEIC는 `pillow-heif` 설치 시)
- 목표 용량 자동 품질 조정 (이진 탐색)
- `--dry-run`: 실제 저장 없이 처리 결과 미리보기
- `--recursive`: 하위 폴더까지 재귀 처리 (서브폴더 구조 유지)
- 10개 이상 파일 처리 시 사전 확인 / 재실행 시 덮어쓰기 or 건너뜀 선택
- 원본 파일 보존 (결과물은 별도 폴더에 저장)
- 크로스플랫폼 지원 (Mac / Linux / Windows)
...
/image-optimizer ./photos iphone --dry-run
/image-optimizer ./gallery 800px --recursive
```
...
- Python 3 (`python3` 또는 `python`) + Pillow — 미설치 시 자동 설치 시도
- Claude Code CLI
