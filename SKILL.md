...
사용자 명령에서 다음을 추출한다:
- **폴더 경로**: 이미지가 있는 디렉토리
- **프리셋 키워드**: `iphone` / `ipad` 중 하나면 프리셋 모드로 진행 (아래 프리셋 목록 참고)
...
Bash 도구로 Pillow 설치 여부를 확인한다:

```bash
python3 -c "import PIL; print('ok')" 2>/dev/null || echo "not_installed"
```

`not_installed`가 출력되면 사용자에게 설치를 안내하고 자동 설치를 시도한다:
...
# ── 프리셋 정의 ──────────────────────────────────────────
PRESETS = {
    'iphone': {'mode': 'cover',      'width': 1284, 'height': 2778},
    'ipad':   {'mode': 'scale_crop', 'width': 2048, 'crop_top': 420, 'crop_bottom': 420},
    'google': {'mode': 'cover',      'width': 1440, 'height': 3040},
}

# ── 파라미터 (Step 1에서 추출한 값으로 교체) ────────────
FOLDER = "[폴더경로]"          # 예: "./photos"
PRESET = None                   # 예: "iphone" 또는 "ipad" (없으면 None)
TARGET_WIDTH = None             # 예: 1200 (프리셋 없을 때, 없으면 None)
TARGET_HEIGHT = None            # 예: None (비율 유지)
TARGET_KB = None                # 예: 1024 (1MB = 1024KB, 없으면 None)
...
if PRESET and PRESET.lower() in PRESETS:
    ...
    PRESET_MODE = cfg['mode']
    TARGET_WIDTH = cfg['width']
    if PRESET_MODE == 'cover':
        TARGET_HEIGHT = cfg['height']
        EXACT_RESIZE = True
        PRESET_LABEL = f"[{PRESET.upper()} 앱스토어 프리셋: {TARGET_WIDTH}×{TARGET_HEIGHT}]"
    else:  # scale_crop
        TARGET_HEIGHT = None
        CROP_TOP = cfg['crop_top']
        CROP_BOTTOM = cfg['crop_bottom']
        EXACT_RESIZE = False
        PRESET_LABEL = f"[{PRESET.upper()} 앱스토어 프리셋: {TARGET_WIDTH}px 너비, 위아래 {CROP_TOP}px 크롭]"
else:
    EXACT_RESIZE = False
...
if OUTPUT_DIR:
    ...
    out_dir = folder.parent / f"{folder.name}_{PRESET.lower()}"
else:
...
| Pillow 미설치 | `pip install Pillow` 실행 안내 |
