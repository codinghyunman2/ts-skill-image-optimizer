...
- 예시:
  ...
  - `/image-optimizer ./screenshots ipad`   → 2048×2732 (애플 앱스토어 iPad)
...
프리셋 사용 시 비율과 무관하게 **정확한 픽셀** 로 출력된다 (cover & center-crop 방식).
...
if PRESET and PRESET.lower() in PRESETS:
    TARGET_WIDTH, TARGET_HEIGHT = PRESETS[PRESET.lower()]
    EXACT_RESIZE = True
...
for img_path in sorted(files):
    try:
        ...
        with Image.open(img_path) as img:
            ...
            upscale_warning = ""
            if EXACT_RESIZE and (original_w < TARGET_WIDTH or original_h < TARGET_HEIGHT):
